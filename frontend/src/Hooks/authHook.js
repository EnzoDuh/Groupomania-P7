...
export const useAuth = () => {
    ...
    //Login useCallback pour ne pas faire une boucle infinie
    const login = useCallback((userId, token, account, expirationDate) => {
        setUserId(userId);
        setToken(token);
        setAccount(account);

        //Créer une date de validité de session d'une durée de 24h
        const tokenExpirationDate = expirationDate || new Date(new Date().getTime() + 1000 * 60 * 60 * 24);
        setTokenExpirationDate(tokenExpirationDate);

        //Envoie des params dans le localstorage
    ...
    //Logout (réinitialisation de toutes les const et suppression du localstorage)
    ...
    //Timer avant expiration de la session
    useEffect(() => {
    ...
    //Auto-log en utilisant les datas du localstorage
    useEffect(() => {
