...
exports.createPost = (req, res, next) => {
    ...
    //Récupération du contenu de la requête et de l'image (si il y en a une)
...
exports.postComment = (req, res, next) => {
    ...
    //Création de la requête pour envoyer le commentaire dans la DB
    const sql = mysql.format(sqlRequests.postComment, [user.id, post_id, validContent]);
    //Requête sql
    db.query(sql, (error, commentId) => {
        if (!error) {
            //Création de la requête pour récupérer le commentaire et lier à son post + les likes et dislikes
